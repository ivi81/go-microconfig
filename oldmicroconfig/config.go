...
// LookupEnvAsSlice получает содержимое переменной окружения ввиде среза строк.
...
func LookupEnvAsInt(key string) (int64, bool) {
	...
	if ok {
		if value, err = strconv.ParseInt(valStr, 10, 64); err != nil {
			log.Println("переменная " + key + " не содержит целое число: " + valStr)
			ok = false
		}
	}
	return value, ok
...
// LookupEnvAsFloat
//
//	Получает содержимое переменной окружения ввиде целого числа.
//	Имя переменной предается в параметре key.
func LookupEnvAsFloat(key string) (float64, bool) {
...
// LookupEnvAsBool получает содержимое переменной окружения ввиде булева значения.
// Имя переменной предается в параметре key.
...
// LookupEnvAsTime получает содержимое переменной окружения ввиде значения time.Duration.
// Имя переменной предается в параметре key.
